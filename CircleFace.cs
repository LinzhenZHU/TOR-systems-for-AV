...
internal class Circle
{
    public Circle(StreamingEmitter emitter)
    {
        _emitter = emitter;
        Radius = 0.012f; // 2cm
        Speed = 5.28f; // 8 metres per second
...
internal static class Example
{
    private static void RunExample(float x, float y, float z, float frequency)
    {
        ...
        DateTimeOffset submissionDeadline)
        {
            foreach (var sample in interval)
            {
                ...
                p.Y = -(float)Math.Sin(phase) * Radius - y + 0.18f;
                ...
                p.Z = p.Z - p.X * (float)Math.Sin(theta) + 0.07f;
    ...
    public static void Main(string[] args)
    {
        ...
        float frequency = 1.0f; // Frequency setting
